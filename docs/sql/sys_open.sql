-- 第三方访问授权管理 DDL（公共能力，sys 前缀）
-- 依据：docs/prepare/功能模块设计-第三方访问授权管理.md
-- 说明：
--   1. 禁止存储客户端 SM2 私钥、服务端 SM2 私钥（服务端密钥仅配置中心/环境变量）
--   2. 逻辑删除：invalid=0 有效；删除时 invalid 置为当前行 id
--   3. 同一 client_id 同时最多一条 key_status=1 的公钥（应用层保证）
...
CREATE TABLE IF NOT EXISTS `sys_open_access_log` (
  `id` bigint NOT NULL COMMENT '主键标识',
  `client_id` varchar(64) NOT NULL COMMENT '客户端标识',
