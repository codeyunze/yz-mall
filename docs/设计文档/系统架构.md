...
根工程：`com.yz:yz-mall:0.0.7-SNAPSHOT`
...
| `mall-demo`（独立工程，路径 `../mall-demo`） | Seata / RocketMQ / Kafka / Canal 等 POC | 已从 yz-mall 拆出；禁止被生产模块依赖 |
...
| Seata | 无 | 仅独立工程 `mall-demo/mall-seata` |
| Canal / Kafka | 无 | 仅 demo |

本地开发中间件参考：`../docker/docker-compose.yml`。

### 10.3 数据与备份（运维）

- 建表脚本：`../script/sql/table-script.sql`
- 备份 / 还原：`../script/shell/mysql-backup-schema-data.sh` 等
- 服务启停：`../script/shell/startup.sh` / `shutdown.sh`
...
| Docker 基建 | `../docker/docker-compose.yml` |
