...
| 数据字典 | mall-sys | 指令类型、告警类型等枚举（车型改为车系/车型主数据表） |
...
| 最新位置 | 写 Redis + MySQL `tw_gps_latest` | P0 |
| 轨迹落库 | 批量写入 **ClickHouse** `tw_gps_track` | P0 |
| 轨迹查询 API | 按 VIN + 时间范围查 ClickHouse | P0 |
| 信号/电量 | 扩展字段或独立消息 | P1 |
...
### 3.8 第三方访问授权（tw-open，P1）

| 功能点 | 说明 | 优先级 |
|--------|------|--------|
| 客户端备案 | clientId、应用信息、启停/过期 | P1 |
| 国密密钥备案 | 客户端 SM2 公钥登记与轮换 | P1 |
| 接口授权 | 按 permission_code 授权可调开放 API | P1 |
| 国密调用通道 | SM2 验签 + SM4 解密 + SM3 完整性 | P1 |
| 调用流水 | 成功/失败审计 | P1 |

> 协议见 [API接口加密认证](https://blog.gaohan.asia/article/z712x92h/)；设计见 [功能模块设计-第三方访问授权管理.md](./功能模块设计-第三方访问授权管理.md)。
...
| 轨迹查询 | tw-telemetry | 轨迹回放 | ClickHouse、Kafka |
