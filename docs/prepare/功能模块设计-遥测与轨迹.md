...
| 项 | 约定 |
|----|------|
| 服务名 | **合入 `mall-tw`**（进程名 `mall-tw`，端口 `25005`）；不再单独部署 `tw-telemetry` |
| 分层 | 与 `mall-tw` 一致：按层分包（`controller` / `service` / `entity` / `mapper` / `dto` / `vo` 等），单模块可运行 JAR |
| 包名 | `com.yz.mall.tw`（类名用前缀区分域） |
| 类前缀 | `TwGps` / `TwTelemetry` |
| 统一响应 | `Result` / `ResultTable` + `ApiController` |
| 鉴权 | Gateway + `@SaCheckPermission("api:tw:telemetry:*")`；数据范围调本进程内车辆 `access/check`（原 `tw-vehicle` Extend） |
| 客户端 | Redis（已有）+ MyBatis-Plus（MySQL）+ ClickHouse JDBC / 官方客户端（批量写入） |

> 历史文档中的独立服务名 `tw-telemetry`、五层 `interface/dao/core/feign/startup` 视为设计演进前的拆分设想；**实现以合入 `mall-tw` 为准**。网关仍按 `/tw/telemetry/**`、`/extend/tw/telemetry/**` 路由到 `lb://mall-tw`。
...
- [x] Docker / 环境：ClickHouse（compose 已加）；Nacos 样例 `docs/nacos/mall-tw-telemetry.yaml`
- [x] MySQL DDL：`tw_gps_latest`（文件已落盘；库内已存在）
- [x] ClickHouse DDL：`tw_gps_track`（文件 + docker init；**容器启动后确认**）
- [x] Kafka 消费组：三写（Redis + MySQL + CH 批量；`tw.telemetry.kafka.enabled` 默认 false）
- [x] API：latest / track / batch（含 Extend latest）
- [x] 权限码 + 调 vehicle access/check（`tw_telemetry_permissions.txt` + `TwTelemetryAccessSupport`）
- [ ] 管理端监控地图 + 轨迹折线（**前端另开，本轮不做**）
- [ ] 与 access 联调：上行 Topic → Kafka（依赖 access 生产侧）
...
## 12. 实现节点（合入 `mall-tw`，每节点可独立验收）

> **原则**：按节点增量交付；每个节点结束时服务可启动（或等价可运行验证），不依赖后续节点才「勉强能跑」。  
> **代码位置**：`yz-mall/mall-tw`。  
> **本轮不做**：`tw-access` / EMQX / Kafka 生产侧；管理端地图页（前端另开）；MySQL 轨迹分表、围栏、在线状态写入。

### 12.1 前提约定

| 项 | 约定 |
|----|------|
| 进程 | `mall-tw`（`25005` / `spring.application.name=mall-tw`） |
| ClickHouse / Kafka | 本地或 Docker 可用；配置走 Nacos / 环境变量，禁止硬编码密钥 |
| 鉴权注解 | 可与全局策略一致先注释 `@SaCheckPermission`；`access/check` 数据范围逻辑仍实现 |
| P1（SOC / 信号） | 表字段预留；读写可随节点 1～3 带上；监控页展示可二期 |
...
| 项 | 内容 |
|----|------|
| 状态 | **仓库侧已完成**（MySQL 表已存在；ClickHouse 待 compose up 后确认 init） |
| 产出 | MySQL `tw_gps_latest` DDL；ClickHouse `tw_gps_track` DDL；`docs/docker` 增加 ClickHouse；`tw.telemetry.*` 配置占位（CH/Kafka 默认 `enabled=false`）；条件装配 `clickHouseDataSource`；网关/Nacos 样例 |
| 落盘 | `mall-tw/src/main/resources/db/tw_gps_latest.sql`；`.../db/clickhouse/tw_gps_track.sql`；`docs/docker/clickhouse/init/`；Harness `.cursor/harness/runs/20260922-tw-telemetry/` |
| 可运行验收 | `mall-tw` 在默认配置下可启动（不强制连 CH/Kafka）；DDL 文件可执行；Compose 可拉起 ClickHouse |

#### 节点 1 — 领域模型 + Latest 写读核心

| 项 | 内容 |
|----|------|
| 状态 | **已完成（仓库侧）** |
| 产出 | Entity / Mapper / Service：`TwGpsLatest` UPSERT；Redis `tw:gps:latest:{vin}`；读路径 Redis → miss MySQL → 回填；无 Kafka 时可用测试写入（单测或临时入口）灌一条假 GPS |
| 落盘 | `TwGpsLatest*`；`TwGpsLatestServiceImpl`；`TwTelemetryDevController`（`POST /tw/telemetry/dev/ingest`、`GET /tw/telemetry/dev/latest`）；坐标校验单测 |
| 可运行验收 | 写入后 Redis 与 MySQL 一致；清空 Redis 后查 latest 能回源并回填 |
...
|----|------|
| 状态 | **已完成（仓库侧）** |
| 产出 | `GET /tw/telemetry/latest`；`POST /tw/telemetry/latest/batch`（≤200）；`GET /extend/tw/telemetry/latest/{vin}`；接车辆访问校验 scope=`2` |
| 落盘 | `TwTelemetryController`；`ExtendTwTelemetryController`；`TwTelemetryServiceImpl`；`TwTelemetryAccessSupport`；Python `mall-tw/src/test/python/tw_telemetry/` |
| 可运行验收 | 用已有 VIN 调最新位置与批量接口，出参符合 `TwGpsLatestVo` |

#### 节点 3 — ClickHouse 批量写 + 轨迹查询

| 项 | 内容 |
|----|------|
| 状态 | **已完成（仓库侧）** |
| 产出 | 本地缓冲批量 INSERT（条数 / 时间双触发 + shutdown flush）；灌点工具或测试方法；`POST /tw/telemetry/track`（时间窗 ≤24h、`maxPoints` 抽稀） |
| 落盘 | `TwClickHouseHttpClient`（HTTP，无 JDBC）；`TwGpsTrackBuffer`；`TwGpsTrackServiceImpl`；`POST /tw/telemetry/track`；dev `track/ingest`/`flush`；抽稀单测 + Python |
| 可运行验收 | 时间窗内折线点序正确；超时间窗拒绝；无数据返回空 `points`（需 `tw.telemetry.clickhouse.enabled=true` + CH 可用） |
...
|----|------|
| 状态 | **已完成（仓库侧）** |
| 产出 | 消费 `tw-telemetry-raw`，组 `tw-telemetry-gps`；解析 JSON、校验经纬度、轻量去噪；顺序 Redis → MySQL UPSERT → 缓冲进 CH；CH 失败不影响 latest；非法坐标丢弃打 warn |
| 落盘 | `spring-kafka`；`TwTelemetryKafkaConfig`（`enabled` 条件）；`TwTelemetryGpsKafkaListener`；`TwTelemetryIngestServiceImpl`；速度去噪；dev `POST /tw/telemetry/dev/raw/ingest` |
| 可运行验收 | `tw.telemetry.kafka.enabled=true` 且配置 `spring.kafka.bootstrap-servers`；Topic 投合法消息后 Latest 有数据；非法坐标不入库；无 Kafka 时可用 raw/ingest 模拟 |
...
|----|------|
| 状态 | **已完成（仓库侧）** |
| 产出 | 权限码 / 菜单 SQL 文本（如需）；API 正例反例脚本或 curl 清单（对齐 §9 T1～T9 后端项）；Harness 落盘 `.cursor/harness/runs/` |
| 落盘 | `db/tw_telemetry_permissions.txt`；Harness `VERIFY_T1_T9.md`；Python 正反例已齐；ACCEPTANCE / TASK 收口 |
| 可运行验收 | 按 `VERIFY_T1_T9.md` 回归；§10 后端项已勾；前端地图与 access 生产联调不在本轮 |
...
- 节点 2 与节点 3 可在节点 1 完成后并行推进，但合并进主线时建议仍按 2 → 3 → 4。  
- 节点 4 需要真实 Kafka；节点 3 允许先用开关 + 手工灌数完成轨迹 API，再联调消费。

### 12.4 与 §10 交付清单映射

| 交付项 | 主要节点 |
|--------|----------|
| Docker / 环境：ClickHouse；Nacos | 0 |
| MySQL DDL：`tw_gps_latest` | 0 |
| ClickHouse DDL：`tw_gps_track` | 0 |
| Kafka 消费组：三写 | 4 |
| API：latest / track / batch | 2、3 |
| 权限码 + access/check | 2、5 |
| 管理端监控地图 + 轨迹折线 | 前端另开（本模块节点不覆盖） |
| 与 access 联调：上行 Topic → Kafka | 4（依赖 access 侧已生产） |
