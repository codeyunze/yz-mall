...
| 时序可选 | MySQL / Redis ZSet /（后续）InfluxDB | 验证阶段轨迹落 MySQL 或按天分表即可 | 简历可写「预留时序库扩展」 |
...
- EMQX 连接/断开 Webhook → `tsp-access` → Redis `tsp:online:{vin}` + RocketMQ `device.online/offline`
...
- 第一阶段：**单库**（可独立 schema `tsp`），单表，预留分表字段
- 高频 GPS：**按日/按月分表**或只保留近 N 天明细 + Redis 最新点
...
| 域 | 表 | 说明 |
|----|-----|------|
| 车辆 | `tsp_vehicle` | VIN、车型、车牌、状态 |
| 车辆 | `tsp_vehicle_owner` | 车辆与用户绑定 |
| 终端 | `tsp_device` | 终端号、密钥摘要、状态 |
| 终端 | `tsp_device_vehicle` | 终端与车辆绑定 |
| 遥测 | `tsp_gps_latest` | 每车最新位置（热数据） |
| 遥测 | `tsp_gps_track_{yyyyMM}` | 轨迹明细（冷数据） |
| 指令 | `tsp_command` | 指令单：类型、参数、状态、超时 |
| 指令 | `tsp_command_log` | 下发/应答流水 |
| 告警 | `tsp_alarm` / `tsp_geofence` | 二期 |

### 7.3 缓存 Key 约定

| Key | 用途 |
|-----|------|
| `tsp:online:{vin}` | 在线标记 + 最后心跳 |
| `tsp:gps:latest:{vin}` | 最新位置快照 |
| `tsp:cmd:idempotent:{bizId}` | 指令幂等 |
| `tsp:device:cred:{clientId}` | 鉴权缓存（短 TTL） |
...
5. 最新位置用 Redis、轨迹用分表/Kafka，冷热如何分？  
