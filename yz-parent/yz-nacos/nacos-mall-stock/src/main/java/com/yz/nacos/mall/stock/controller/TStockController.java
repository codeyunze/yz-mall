...
public class TStockController extends ApiController {
    ...
    @Autowired
    void tStockController(TStockService service) {
     ...
     */
    @PostMapping("/page")
    public TableResult<List<TStock>> page(@RequestBody @Validated PageFilter<TStock> filter) {
        Page<TStock> page = this.tStockService.page(new Page<>(filter.getCurrent(), filter.getSize()), new QueryWrapper<>(filter.getFilter()));
     ...
     */
    @GetMapping("/get/{id}")
    public Result<TStock> selectOne(@PathVariable Serializable id) {
     ...
     */
    @PostMapping("/add")
    public Result<Boolean> insert(@RequestBody @Validated TStock tStock) {
     ...
     */
    @PostMapping("/update")
    public Result<Boolean> update(@RequestBody @Validated TStock tStock) {
