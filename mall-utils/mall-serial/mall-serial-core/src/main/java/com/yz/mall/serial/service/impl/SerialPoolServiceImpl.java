 ...
 * @version 3.0
 */
@Service("serialPoolServiceImpl")
public class SerialPoolServiceImpl extends SerialServiceImpl {
    ...
    public String generateSerialNumber(String prefix, Integer numberLength) {
        // 直接从号池获取流水号
        SerialNumberDto serialNumberDto = SerialHolder.get(prefix);
        ...
        try {
            ...
            SerialNumberDto secondSerialNumberDto = SerialHolder.get(prefix);
            ...
            for (int i = 0; i < numberPoolSize; i++) {
                ...
                // 存入流水号号池
                SerialHolder.add(prefix, new SerialNumberDto(code, currentSerialNumber));
            }

            // 从号池获取流水号
            return Objects.requireNonNull(SerialHolder.get(prefix)).getCode();
        } finally {
            redissonLock.unlock();
        }
    }
