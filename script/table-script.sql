...
(
    ...
    frame_loading    INT               DEFAULT 1 COMMENT '加载动画(内嵌的iframe页面是否开启首次加载动画)(1:开启,0:关闭)',
    keep_alive       INT               DEFAULT 0 COMMENT '缓存页面(是否缓存该路由页面)(1:缓存,0:不缓存)',
    hidden_tag       INT               DEFAULT 1 COMMENT '标签页(当前菜单名称或自定义信息禁止添加到标签页)(1:禁止,0:允许)',
    fixed_tag        INT               DEFAULT 0 COMMENT '固定标签页(当前菜单名称是否固定显示在标签页且不可关闭)(1:固定,0:不固定)',
    show_link        INT               DEFAULT 1 COMMENT '是否显示该菜单(1:显示,0:不显示)',
    show_parent      INT               DEFAULT 0 COMMENT '是否显示父级菜单(1:显示,0:不显示)',
    PRIMARY KEY (id)
) COMMENT = '系统-菜单资源表';
